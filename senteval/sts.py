...
class STSEval(object):
    ...
    def run(self, params, batcher):
            ...
            for ii in range(0, len(gs_scores), params.batch_size):
                ...
                if len(batch1) == len(batch2) and len(batch1) > 0:
                    ...
                    for kk in range(len(enc2)):
                        ...
                        avg1 = np.mean(enc1[kk], axis=0)
                        avg2 = np.mean(enc2[kk], axis=0)

                        _, p1 = shapiro(avg1)
                        if p1 < 0.05:
                            reject_normal_count += 1
        ...
        logging.debug('ALL (average) : Pearson = %.4f, \
            Spearman = %.4f' % (avg_pearson, avg_spearman))
        logging.debug('Fraction of sentences where normality was not rejected'
                      ' = %.4f\n' % (1 - 1.0 * reject_normal_count / total_count))
